...
class DevMenuAppInstance: NSObject, RCTBridgeDelegate {
  ...
  func sourceURL(for bridge: RCTBridge!) -> URL! {
    ...
    if let packagerHost = jsPackagerHost() {
      ...
      print("Expo DevMenu packager host %@ not found, falling back to bundled source file...", packagerHost);
    }
    #endif
    return jsSourceUrl()
  }
  ...
  func extraModules(for bridge: RCTBridge!) -> [RCTBridgeModule]! {
    let internalModule = DevMenuInternalModule(manager: self.manager)
  ...
  private func jsPackagerHost() -> String? {
    ...
    return try? String(contentsOfFile: packagerHostPath, encoding: String.Encoding.utf8).trimmingCharacters(in: CharacterSet.newlines)
  }
}
